...
# Dendritic style – pure config, no options.
{ config, lib, pkgs, ... }:

{
  ...
  services.crowdsec = {
    ...
    # Local API only – never expose this
    settings = {
      general = {
        api.server = {
          enable = true;
  ...
  ##########################################################################
  # Firewall Bouncer (enforces the bans)
  ##########################################################################
  services.crowdsec-firewall-bouncer = {
    enable = true;

    # The module will auto-register the bouncer when possible.
    # If you prefer a fixed key you can set:
